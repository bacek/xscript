...
std::string
Pcre::replace(const std::string &piece, const std::string &with) {
    /*
    * Pcre::replace version by "Marcus Kramer" <marcus.kramer@scherrer.de>
    */
    std::string Replaced(piece);
    
    bool bReplaced = false;
    int  iReplaced = -1;
    
    __pcredebug << "replace: " << piece << " with: " << with << std::endl;
    ...
    */
    ...
    __pcredebug << "_expression: " << _expression << std::endl;
    Compile(_flags);
    ...
    if (search(piece)) {
        ...
        if (!global_t) {
            ...
            if(matched() && matches() >= 1) {
                __pcredebug << "matches: " << matches() << std::endl;
