...
Pcre::_split(const std::string &piece, int limit, int start_offset, int end_offset) {
    std::vector<std::string> Splitted;
    /* _expression will be used as delimiter */
    if (_expression.length() == 1) {
        /* use the plain c++ way, ignore the pre-compiled p_pcre */
        std::string buffer, _delimiter, _piece;
        char z;
        if (case_t) {
        ...
        for(;;) {
            if (search(piece, pos) == true) {
                if (matches() > 0) {
                    ...
                    std::string junk(piece, piece_start, piece_end - piece_start + 1);
                    num_pieces++;
                ...
                std::string junk(piece, pos, (piece.length() - pos));
                ++num_pieces;
                if ((limit != 0 && num_pieces < limit) || limit == 0) {
                    if ((start_offset != 0 && num_pieces >= start_offset) || start_offset == 0) {
                        if ((end_offset != 0 && num_pieces <= end_offset) || end_offset == 0) {
...
std::vector<std::string>
Pcre::split(const std::string &piece) {
    return _split(piece, 0, 0, 0);
}

std::vector<std::string>
Pcre::split(const std::string &piece, int limit) {
    return _split(piece, limit, 0, 0);
}

std::vector<std::string>
Pcre::split(const std::string &piece, int limit, int start_offset) {
    return _split(piece, limit, start_offset, 0);
}

std::vector<std::string>
Pcre::split(const std::string &piece, int limit, int start_offset, int end_offset) {
    return _split(piece, limit, start_offset, end_offset);
}

std::vector<std::string>
Pcre::split(const std::string &piece, std::vector<int> positions) {
    std::vector<std::string> PreSplitted = _split(piece, 0, 0, 0);
    std::vector<std::string> Splitted;
    for(std::vector<int>::iterator vecIt = positions.begin(); vecIt != positions.end(); ++vecIt) {
