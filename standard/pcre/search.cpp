 ...
 */
bool
Pcre::search(const std::string &stuff, int OffSet){
    return dosearch(stuff, OffSet);
}

bool
Pcre::search(const std::string &stuff){
    return dosearch(stuff, 0);
}

bool
Pcre::dosearch(const std::string &stuff, int OffSet){
    reset();
    if (sub_vec != NULL) {
        delete[] sub_vec;
    }
    ...
    __pcredebug << "Pcre::dosearch(): pcre_exec() returned: " << num << std::endl;

    if(num < 0) {
        /* no match at all */
        __pcredebug << " - no match" << std::endl;
        return false;
    }
    else if (num == 0) {
        /* vector too small, there were too many substrings in stuff */
        __pcredebug << " - too many substrings" << std::endl;
        return false;
    }
    else if (num == 1) {
        /* we had a match, but without substrings */
        __pcredebug << " - match without substrings" << std::endl;
        did_match = true;
        num_matches = 0;
        return true;
    }
        ...
        resultset = new std::vector<std::string>;
        const char **stringlist;
        did_match = true;
        num_matches = num - 1;

        __pcredebug << " - match with " << num_matches << " substrings" << std::endl;

        int res = pcre_get_substring_list((char *)stuff.c_str(), sub_vec, num, &stringlist);
        if(res == 0) {
            __pcredebug << "Pcre::dosearch(): matched substrings: " << std::endl;
            for(int i = 1; i < num; ++i) {
                __pcredebug << " " << std::string(stringlist[i]) << std::endl;
        ...
        __pcredebug << " - uncommon error" << std::endl;
        return false;
    }
}
