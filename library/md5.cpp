...
static void MD5Transform(boost::uint32_t[4], const unsigned char[64]);
static void Encode(unsigned char*, boost::uint32_t*, unsigned int);
static void Decode(boost::uint32_t*, const unsigned char*, unsigned int);
...
void MD5_Update(MD5_CTX *context, const unsigned char *input, unsigned int inputLen) {
	...
	if ((context->count[0] += ((boost::uint32_t)inputLen << 3)) < ((boost::uint32_t)inputLen << 3))
		context->count[1]++;
	...
	if (inputLen >= partLen) {
		memcpy((unsigned char *)&context->buffer[ndx], (unsigned char *)input, partLen);
	...
	memcpy((unsigned char *)&context->buffer[ndx], (unsigned char *)&input[i], inputLen-i);
}
...
static void MD5Transform(boost::uint32_t state[4], const unsigned char block[64]) {
...
/* Encodes input (boost::uint32_t) into output (unsigned char). Assumes len is
  a multiple of 4.
 */
static void Encode(unsigned char *output, boost::uint32_t *input, unsigned int len) {
	unsigned int i, j;
...
/* Decodes input (unsigned char) into output (boost::uint32_t). Assumes len is
  a multiple of 4.
 */
